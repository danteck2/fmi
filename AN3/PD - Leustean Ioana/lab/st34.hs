...
onlyDigit xs = xs == (filter isDigit xs )
...
getNo :: String -> Int
getNo xs =let y =(\x-> (digitToInt x) *10) in foldr (+y) 0 xs
...
removeNotDigit xs = filter isDigit xs
...
parseTerm "" = 0
parseTerm (x:xs) | isDigit x && isDigit (head xs)= ( fromIntegral (digitToInt x) * (10^(length xs))) + parseTerm xs
		 | isDigit x  = ( fromIntegral (digitToInt x) * (10^(length xs))) 'div' (10^((length xs) -1)
		 |otherwise = 0
-- 6 (0.5pt).
...
splitWhenPlus xs = splitWhen (\x->x=='+') xs
...
-- Nu recursiv.
eval :: String -> Integer
eval xs = foldr (+(\x->parseTerm x)) 0 (splitWhenPlus xs)
