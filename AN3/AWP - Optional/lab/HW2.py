...
class CarRental(object):
    ...
    def get_cars(self, brand=None, max_price=None):
        if brand == None and max_price == None:
            return self.cars
        if brand == None:
            return [car for car in self.cars if car.daily_price <= max_price]
        if max_price == None:
            return [car for car in self.cars if car.brand == brand]
        return [car for car in self.cars if car.brand == brand and car.daily_price <= max_price]
...
def main():
    ...
    p = Person('Jane', 'Geller', date(2015, 12, 5))
